...
const InputControl = ({ isSignup }: { isSignup?: boolean }) => {
    const { errors, isboolean, userCredntial, active,
        setActive, setIsBoolean, setErrors, setUserCredential } = usePreLoginStore()
    ...
    const loginResponseValidation = async () => {
        ...
        const getUser = pwdDeocoded.find((e: { mail: string }) => e.mail.toLocaleLowerCase().includes(user.mail.toLocaleLowerCase()))
        const checkMail = getUser?.mail.toLocaleLowerCase() === user.mail.toLocaleLowerCase()
        ...
        <div>
                ...
                <>
                    <div className='mt-3'>
                        <input type='text' className='form-control' name='name' placeholder='Enter Name'
                            onChange={handleChange} value={userCredntial[fieldSate]?.name || ''} />
                    </div>
            ...
            <div className={`${!isSignup && 'mt-3'}`}>
                <input type="email" className="form-control" name='mail' placeholder="Enter email"
                    onChange={handleChange} value={userCredntial[fieldSate]?.mail || ''} />
            </div>
            <p className='mb-2 pwd-txt'>{errors[fieldSate]?.mail}</p>
            <div className='position-relative'>
                <input type={!isboolean.pwdIconShow ? "password" : 'text'} className="form-control" name='pwd'
                    placeholder="Enter password" onChange={handleChange} value={userCredntial[fieldSate]?.pwd || ''} />
                <div className='position-absolute h-100 d-flex align-items-center justify-content-center top-0'
                    style={{ width: '5%', right: '4%' }}>
                    <img style={{ cursor: 'pointer' }} src={!isboolean.pwdIconShow ? eyeClose : eyeOpen}
                        className='img-fluid' onClick={() => setIsBoolean({ pwdIconShow: !isboolean.pwdIconShow })} />
                </div>
            </div>
            <div className='d-flex'>
                ...
                {errors[fieldSate]?.pwd === 'update' &&
                    <>
                        <div className='pwd-img-container d-flex align-items-start ms-1'
